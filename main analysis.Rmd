...
# Identifying models
delay <- Sys.time()
y_hat = regression_forest(data_s %>% select(id_filter %>% pull) %>% mutate_all(as.numeric)%>% select(where(not_all_na)) %>% missMethods::impute_median(), Y%>% missMethods::impute_median() %>% pull(), num.trees = 10000)
Sys.time() - delay
w_hat = regression_forest(data_s %>% select(id_filter %>% pull) %>% mutate_all(as.numeric)%>% select(where(not_all_na)) %>% missMethods::impute_median(), W%>% missMethods::impute_median() %>% pull(), num.trees = 10000)
```


```{r}
# Full population
full_cf <- causal_forest(X, 
...
ggplot(data = data_s, aes(highest_ed, get_scores(full_cf))) +
  ...
  ggforce::facet_zoom(ylim = c(-5000,5000)) +
